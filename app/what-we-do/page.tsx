  ...
  return (
    <main className={`min-h-screen flex flex-col ${d ? "bg-black text-white" : "bg-white text-black"}`}>
      ...
      <section className="px-6 md:px-16 lg:px-24 pt-8 pb-16 md:pb-20">
        <p className="text-[11px] uppercase tracking-[0.3em] text-gray-400 mb-12">What We Do</p>
        <h1 className="text-[clamp(3rem,8vw,7rem)] font-medium leading-[0.95] tracking-[-0.03em] mb-10">
          O que fazemos<br />
          <em className="font-extralight italic">é inseparável de como pensamos.</em>
        </h1>
      ...
      <section className="px-6 md:px-16 lg:px-24 pb-20 md:pb-32">
        <p className={`text-xl md:text-2xl leading-[1.75] max-w-4xl ${d ? "text-gray-400" : "text-gray-600"}`}>
          Cada área que ativamos é parte de um todo coerente — porque uma marca que não é coerente internamente nunca o será externamente. Trabalhamos de forma integrada ou de forma focada, consoante o que cada marca precisa. Mas o princípio é sempre o mesmo: nada existe por acaso, e nada funciona sozinho.
        </p>
        <p className={`text-xl md:text-2xl leading-[1.75] max-w-4xl mt-6 font-light italic ${d ? "text-gray-500" : "text-gray-400"}`}>
          O que se segue não é um catálogo. É uma descrição de como pensamos o trabalho — e do que está disponível para quem quiser construir algo que dure.
        </p>
      </section>
          ...
          {services.map((s) => (
            <div key={s.number} className="reveal py-16 md:py-20">
              <div className="flex items-baseline gap-4 mb-8">
                <span className={`text-3xl md:text-5xl font-medium tracking-tight shrink-0 ${d ? "text-gray-700" : "text-gray-200"}`}>{s.number}</span>
                <h2 className="text-3xl md:text-5xl font-medium tracking-[-0.02em]">{s.title}</h2>
              </div>
              <div className="max-w-3xl mb-10">
                <p className="text-xl md:text-2xl font-medium mb-4">{s.tagline}</p>
                <p className={`text-lg md:text-xl leading-[1.75] ${d ? "text-gray-400" : "text-gray-600"}`}>{s.description}</p>
              </div>
              <div className="flex flex-wrap items-center gap-x-2 gap-y-2">
                {s.tags.map((tag, i) => (
                  <span key={tag} className="flex items-center gap-2">
                    <span className={`text-sm ${d ? "text-gray-500" : "text-gray-400"}`}>{tag}</span>
                    {i < s.tags.length - 1 && <span className={`text-base leading-none ${d ? "text-gray-700" : "text-gray-300"}`}>·</span>}
                  </span>
                ))}
              </div>
            </div>
      ...
      <section className="px-6 md:px-16 lg:px-24 pb-32 md:pb-48 flex flex-col items-start gap-8">
        <p className="text-2xl md:text-4xl font-medium leading-[1.1] tracking-[-0.02em]">
          Não fazemos tudo.<br />
          <em className="font-extralight italic text-gray-400">Fazemos apenas o que importa.</em>
        </p>
